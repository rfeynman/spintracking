...
def spin(): #0.3(B*L)/p
    ...
    for i in range(nstep):
        stepphase=0.3*magnet_bvsz(z_ini+i*dstep)/beam_pvsz(z_ini+i*dstep)*dstep+phase[-1]  #rad
        phase.append(stepphase)
        z.append(z_ini+i*dstep)
        b.append(magnet_bvsz(z_ini+i*dstep))
    #print(phase)
    spin_phase=np.array(phase)
    spin_z=np.array(z)
    spin_phasevsz=np.column_stack((spin_z,spin_phase))
    spin_phasevsz=np.column_stack((spin_phasevsz,b))

    #spin_phasevsz[:,1]= spin_phasevsz[:,1]*180/3.1415927
    print(spin_phasevsz) 
...
def figure_plot():
    ...
    ax3.set_xlabel('z[cm]')
    ax3.set_ylabel('spin angle[rad]',color='r')
